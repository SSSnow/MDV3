...
uint8_t SCCB_ReadMem(uint8_t slv_addr, uint8_t reg)
{
    uint8_t data = 0;
    uint8_t length = 1;
    ...
    HAL_I2C_Mem_Read(&I2CHandle, slv_addr, reg, I2C_MEMADD_SIZE_8BIT, &data, length, HAL_MAX_DELAY);
...
uint8_t SCCB_WriteMem(uint8_t slv_addr, uint8_t reg, uint8_t data)
{
    uint8_t ret = 0;

    __disable_irq();

    if (HAL_I2C_Mem_Write(&I2CHandle, slv_addr, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, HAL_MAX_DELAY) != HAL_OK) {
        ret=0xFF;
    }
